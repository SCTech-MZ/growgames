...
export class AparelhoService{
    constructor(
        private aparelhorepo: AparelhoRepository,
        private notificacaoservice: NotificacaoService,
        private atividadeService: AtividadeService){ }
    
    
    async listar(): Promise<IAparelho[]>{
    ...
    async create(dados: Partial<IAparelho>, perfil: string): Promise<IAparelho>{
    ...
    async update(id: number, dados: Partial<IAparelho>, perfil: string): Promise<IAparelho>{
    ...
    async remover(id: number, perfil: string) {
        const aparelho = await this.aparelhorepo.findById(id);
        if (!aparelho) throw new Error("Aparelho nao encontraddo");
    ...
    async listarPrecisaManutencao(): Promise<IAparelho[]>{
